...
//    }
//    if (([lastDate hour]+8)>=5 && ([lastDate hour]+8)<12) {
//        period = @"AM";
//        hour = [NSString stringWithFormat:@"%02d",(int)([lastDate hour]+8)];
//    }else if (([lastDate hour]+8)>=12 && ([lastDate hour]+8)<=18){
//        period = @"PM";
//        hour = [NSString stringWithFormat:@"%02d",(int)([lastDate hour]+8)-12];
//    }else if (([lastDate hour]+8)>18 && ([lastDate hour]+8)<=23){
//        period = @"Night";
//        hour = [NSString stringWithFormat:@"%02d",(int)([lastDate hour]+8)-12];
//    }else{
//        period = @"Dawn";
//        hour = [NSString stringWithFormat:@"%02d",(int)([lastDate hour]+8)];
//    }
//    NSString *realHour = [NSString stringWithFormat:@"%ld",[lastDate hour]+8];
    if ([lastDate year]==[[NSDate date] year]) {
        NSInteger days = [NSDate daysOffsetBetweenStartDate:lastDate endDate:[NSDate date]];
        if (days <= 2) {
            dateStr = [lastDate stringYearMonthDayCompareToday];
